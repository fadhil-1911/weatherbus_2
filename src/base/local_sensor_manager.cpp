...
bool LocalSensorManager::setupSHT41() {
    ...
    if (Wire.endTransmission() != 0) {
        Serial.println("ERROR: Local SHT41 communication failed");
        return false;
    }
    delay(10);
    Serial.println(
...
bool LocalSensorManager::readSHT41(float& temperature, float& humidity) {
    ...
    if (received != 6) {
        Serial.println("ERROR: Local SHT41 invalid response length");
        return false;
    }
    uint8_t data[6];
    ...
    if (sht41Crc8(data, 2) != data[2]) {
        Serial.println("ERROR: Local SHT41 temperature CRC failed");
        return false;
    }
    // Humidity CRC
...
bool LocalSensorManager::setupBME280() {
    ...
    Serial.println("Local BME280 communication OK");
    return true;
}
...
bool LocalSensorManager::recoverBME280() {
    ...
    if (!bme.begin(BME280_I2C_ADDRESS, &Wire)) {
        Serial.println("Local BME280 recovery failed");
        return false;
    }
    Serial.println("Local BME280 recovery OK");
    return true;
}
...
bool LocalSensorManager::readBME280(float& pressure) {
    ...
    if (!isfinite(reading) || reading < 300.0F || reading > 1100.0F) {
        Serial.println("ERROR: Local BME280 pressure invalid");
        if (!recoverBME280()) {
            return false;
        }
        reading = bme.readPressure() / 100.0F;
        if (!isfinite(reading) || reading < 300.0F || reading > 1100.0F) {
            Serial.println("ERROR: Local BME280 pressure still invalid after recovery");
            return false;
        }
    }
    pressure = reading;
...
bool LocalSensorManager::begin() {
    ...
    Serial.println("--------------------");
    Serial.printf("SHT41  : %s\n", ENABLE_SHT41 ? "ENABLED" : "DISABLED");
    Serial.printf("BME280 : %s\n", ENABLE_BME280 ? "ENABLED" : "DISABLED");
    Serial.println("I2C SDA: GPIO 8");
    Serial.println("I2C SCL: GPIO 9");
    Serial.println();
    ...
    if (ENABLE_SHT41) {
        ...
        if (!sht41Available) {
            Serial.println("SHT41 unavailable - continuing");
        }
    }
    ...
    if (ENABLE_BME280) {
        ...
        if (!bme280Available) {
            Serial.println("BME280 unavailable - continuing");
        }
    }
    Serial.println();
    return true;
...
bool LocalSensorManager::readSensors(
    ...
    if (ENABLE_SHT41 && sht41Available) {
        if (readSHT41(data.temperature, data.humidity)) {
            ...
            flags |= WeatherBus::FLAG_HUMIDITY_VALID;
        } else {
            Serial.println("WARNING: Local SHT41 read failed");
        }
    }
    ...
    if (ENABLE_BME280 && bme280Available) {
        if (readBME280(data.pressure)) {
            ...
            Serial.println("WARNING: Local BME280 read failed");
        }
    }
    return flags != 0;
}